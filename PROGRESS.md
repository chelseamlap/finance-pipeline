...
BigQuery (`spending-pipeline.finance_pipeline`) is the durable, queryable store. Four views answer the recurring "how much did we spend on X each month" questions without double-counting item-level detail against Simplifi: `v_item_level_monthly_category`, `v_simplifi_monthly_category`, `v_blended_monthly_category`, `v_monthly_category_totals`.

## Recent Work (2026-08-19 through 2026-08-21)
...
- **Budget category design** (sinking vs. fixed-required vs. true-discretionary) is still an open conversation — `One-Time Project` is currently doing double duty as a de facto "sinking fund" bucket (Home_Improvement, Gifts, Travel) but isn't quite that concept. Gas_Auto also mixes routine fuel with lumpy repair/tire spend. Discussed but not yet restructured.
