...
Before categorization and reconciliation, same-order duplicate OrderPro item rows are collapsed when the stable item fields, item amount fields, and retailer source totals match. This removes repeated Costco OrderPro rows while preserving rows that differ by quantity, price, subtotal, or source totals.
